...
function checkStock()
    ...
    local quantity = {}
    file = io.open("DataFiles/ShopMenu.txt","r")
    for line in file:lines() do
        if line ~= nil then
    ...
    local total = {}
    counter = 1
    file = io.open("DataFiles/ItemData.txt","r")
    for line in file:lines() do
        if line ~= nil then
    ...
    for index,item in ipairs(quantity) do
        print(index, item)
    end
    ...
    print("result")
    for _,x in ipairs(returnList) do
        print(x)
    end
